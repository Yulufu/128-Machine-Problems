...
public:
  ...
  // helper
  void ResizeColInRows();
  void CastinAddCol(const std::pair<std::string, DataType>& col_desc);
  void CastinDeleteCol(unsigned int col_idx);
  void CastinAddRow(const std::initializer_list<std::string>& col_data,
                    int& row_num);
  void CastinDeleteRow(unsigned int id);
...
// void** --> an address that store bunch of addresses to void objects